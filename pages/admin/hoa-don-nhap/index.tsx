...
import getManyImportOrderRequest from "../../../apiRequests/importOrder/getManyImportOrderRequest";
...
import deleteManyImportOrderRequest from "../../../apiRequests/importOrder/deleteManyImportOrderRequest";
...
const columns: GridColDef[] = [
  ...
  },
  {
    ...
    renderCell: (params: GridRenderCellParams<string>) => (
      <Stack>
        <Link href={`/admin/hoa-don-nhap/detail/${params.id}`} passHref>
          <IconButton>
...
const ImportOrderPage: CustomNextPage = () => {
  ...
  //========Queries====================
  const getManyImportOrderQuery = useQuery(
    ["getManyImportOrder", { limit, offset }],
    () => {
      let ids = [];
  ...
  const deleteManyImportOrder = useMutation(
    (ids: string[]) =>
      deleteManyImportOrderRequest({
        ids: ids,
        accessToken: session.data?.user?.accessToken,
      }),
    {
