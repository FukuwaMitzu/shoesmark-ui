...
const columns: GridColDef[] = [
...

  {
    ...
    renderCell: (params: GridRenderCellParams<string>) => (
      <Typography>
        {currencyFormater.format(parseFloat(params.value ?? ""))}
      </Typography>
    ),
  },
  {
  ...
  },
  {
    ...
    renderCell: (params: GridRenderCellParams<string>) => (
      <Stack>
        <Link href={`/admin/giay/detail/${params.id}`} passHref>
          <IconButton>
...
const ShoesPage: CustomNextPage = () => {
  ...
  const getAllShoesQuery = useQuery(
    ["getAllShoes", pagination.offset, pagination.limit],
    () =>
      getAllShoesRequest({
  ...
  const deleteSelectedQuery = useMutation(
    (ids: string[]) =>
      deleteManyShoesRequest({
        ids: ids,
